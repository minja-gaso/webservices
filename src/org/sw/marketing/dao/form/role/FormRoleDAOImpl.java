...
public class FormRoleDAOImpl extends BaseDAO implements FormRoleDAO
{
	@Override
	public void insert(Role role)
	{
		...
		try
		{
			...
			statement = connection.prepareStatement(FormSQL.INSERT_ROLE);
	...
	public void delete(long roleID)
	{
		...
		{
			connection = dao.getConnection();
			statement = connection.prepareStatement(FormSQL.DELETE_CALENDAR_ROLE);
	...
	public Role getUniqueRole(Role role)
	{
		...
		{
			connection = dao.getConnection();
			statement = connection.prepareStatement(FormSQL.GET_FORM_ROLE_UNIQUE_CHECK);
			statement.setString(1, role.getType());
			statement.setString(2, role.getEmail());
			statement.setLong(3, role.getFkId());
			...
			while (resultSet.next())
			{
				...
				long fkCalendarId = resultSet.getLong("fk_form_id");
	...
	public List<Role> getRoles(long calendarID)
	{
		...
		{
			connection = dao.getConnection();
			statement = connection.prepareStatement(FormSQL.GET_FORM_ROLES);

...
import org.sw.marketing.dao.skin.SkinSQL;
import org.sw.marketing.data.skin.Skin.Role;

public class SkinRoleDAOImpl extends BaseDAO implements SkinRoleDAO
{
	@Override
	public void insert(Role role)
	{
		...
		try
		{
			...
			statement = connection.prepareStatement(SkinSQL.INSERT_ROLE);
	...
	public void delete(long roleID)
	{
		...
		{
			connection = dao.getConnection();
			statement = connection.prepareStatement(SkinSQL.DELETE_ROLE);
	...
	public Role getUniqueRole(Role role)
	{
		...
		{
			connection = dao.getConnection();
			statement = connection.prepareStatement(SkinSQL.GET_ROLE_UNIQUE_CHECK);
			statement.setString(1, role.getType());
			statement.setString(2, role.getEmail());
			statement.setLong(3, role.getFkId());
			...
			while (resultSet.next())
			{
				...
				long fkId = resultSet.getLong("fk_skin_id");

				uniqueRole = new Role();
				uniqueRole.setId(id);
				uniqueRole.setType(type);
				uniqueRole.setEmail(email);
				uniqueRole.setFkId(fkId);
			}
		}
		catch (SQLException e)
		{
	...
	public List<Role> getRoles(long calendarID)
	{
		...
		{
			connection = dao.getConnection();
			statement = connection.prepareStatement(SkinSQL.GET_ROLES);
